...
const Comment = (props) => {
  ...
  const handleReyakks = async () => {
    try {
      const { data } = await axiosRes.post("/comment_reyakks/", {
        comment: id,
      });
      setComments((prevComments) => ({
        ...
        results: prevComments.results.map((comment) => {
          return comment.id === id
            ? {
                ...comment,
                comment_reyakks_count: comment.comment_reyakks_count + 1,
                comment_reyakks_id: data.id,
              }
            : comment;
        }),
      }));
      ...
      <Card.Header>
        ...
        <div className="text-right">
          {is_commenter ? (
            <OverlayTrigger
              placement="top"
              overlay={<Tooltip>You can't reyakk your own comments!</Tooltip>}
            >
              <i className="far fa-heart" />
            </OverlayTrigger>
          ) : comment_reyakks_id ? (
            <span onClick={handleUnReyakks}>
              <i className={`fa-solid fa-heart ${reyakkStyles.Reyakked}`}></i>
            </span>
          ) : currentUser ? (
            <span onClick={handleReyakks}>
              <i className={`far fa-heart ${reyakkStyles.ReyakkHover}`} />
            </span>
          ) : (
            <OverlayTrigger
              placement="top"
        ...
        {is_commenter && !editForm && (
          <>
            <div className="text-right">
              <OverlayTrigger
                placement="top"
                overlay={<Tooltip>Click to edit comment</Tooltip>}
              >
                <i
              ...
              <OverlayTrigger
                placement="top"
                overlay={<Tooltip>Click to delete comment</Tooltip>}
              >
                <i
