...
import {
  useSetYakfileData,
  useYakfileData,
} from "../../contexts/YakfileDataContext";
...
function YakfilePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [{ data: pageYakfile }, { data: yakfilePosts }] =
          await Promise.all([
          ...
          {yakfile?.is_author && (
            <>
              <div>
                <OverlayTrigger
                  placement="top"
                  overlay={<Tooltip>Click to edit yakfile</Tooltip>}
                >
                  <i
                    className={`fa-solid fa-file-pen ${styles.EditDeleteIcon}`}
                    onClick={() => {
                      navigate(`/yakfile/${id}/edit`);
                    }}
                  ></i>
                </OverlayTrigger>
                ...
                >
                  <i
                    ...
                    onClick={() => {
                      navigate(`/yakfile/${id}/edit/username`);
                    }}
                  ></i>
                </OverlayTrigger>
                <span className="text-dark ml-1 mr-1"> | </span>
                ...
                >
                  <i
                    ...
                    onClick={() => {
                      navigate(`/yakfile/${id}/edit/password`);
                    }}
                  ></i>
                </OverlayTrigger>
              </div>
      ...
      {yakfilePosts.results.length ? (
        ...
        <Container className={"text-center d-flex justify-content-center p-4"}>
          <Asset
            ...
            height={100}
            width={100}
            message={`No results found, ${yakfile?.author} hasn't posted yet.`}
          />
        </Container>
      )}
    </>
